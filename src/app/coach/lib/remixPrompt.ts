...
export function buildRemixPrompt(post: LoggedPost): string {
  ...
  lines.push(
    "You are remixing one of my high-performing Instagram carousels. The original post outperformed my baseline — the underlying ARGUMENT and FACTS resonated with my audience. I want to repost the same argument 60-90 days from now in noticeably different words so a returning viewer thinks \"this is a different post on a similar topic\" — not \"I've seen this before.\"",
    "",
    "Your job: produce ONE ready-to-publish remix. Same argument, same facts, same CTA, but freshly written from scratch. I do NOT want a menu of options — I want the single best version, ready to paste straight into my slide builder.",
    "",
    "WHAT TO PRESERVE (facts and anchor points — these are non-negotiable):",
    "- Same number of slides, same role per slide (cover, story, criteria, ranked items, etc.)",
    "- Same point per slide — slide 3 makes the same argument it made before",
    "- Every dollar amount, percentage, year — those are facts, don't fabricate or drop them",
    "- Every named city, person, brand — same entities",
    "- The CTA on the final slide, reproduced verbatim",
    "",
    "WHAT TO REWRITE (everything else — be substantively different, not cosmetic):",
    "- Cover hook: brand new headline + optional subtitle",
    "- Every body-slide headline: rephrase from scratch — different sentence shape, different verbs, different framing of the same point",
    "- Every bullet: rewrite the wording, reorder the sequence inside the slide, change which words are bolded. Two bullets that say the same thing should READ differently from the original.",
    "- Sentence structure throughout: if the original led with a stat, lead with the implication this time. If it asked, assert. If it was three short bullets, try one slightly longer one + two punchy ones.",
    "- Bolding pattern: keep roughly the same density (2-4 bolded spans per slide) but pick DIFFERENT high-impact words to emphasize — usually the numbers and the verbs, but pick fresh ones.",
    "",
    "TEST before you output: read your remix slide-by-slide and ask \"would I recognize this as the same post if I scrolled past it?\" If any slide reads >50% identical to the original, rewrite it again with a different sentence shape.",
    "",
    ...
    "2. Body slides (slides 2 through N-1): see WHAT TO REWRITE above. Same point per slide, same facts, but visibly different writing.",
    "3. CTA (final slide): LOCKED. Reproduce verbatim. Do not rephrase. That part converted.",
    "4. Preserve the voice and bolding density (~2-4 **bolded** spans per slide), but pick fresh words to bold.",
    "5. Do NOT introduce new claims, new numbers, or new named entities not in the original. Remix the wording, not the substance.",
    "",
